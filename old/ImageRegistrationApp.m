function [outImage] = ImageRegistrationApp(movingInputImage,fixedInputImage)
...
if ischar(fixedInputImage)
    ...
    size_fixedInputImage = size(fixedInputImage)
end
...
RegistrationConfigButtonGroup = uibuttongroup(...
    ...
    'Position',[0.225 0.205 0.105 0.160],...
	'BackgroundColor',bgc,...
    'SelectedObject',[],...
    'SelectionChangeFcn',@changeTformType,...
    'OldSelectedObject',[]);
...
PreOptsPanel = uipanel(...
    ...
    'Bordertype','none',...
    'Position',[0 0.025 0.95 0.95]);
tts = 'Rotate moving image.';
tts = sprintf(wrapString(tts,ttStringLength));
[RotationSlider,RotationPanel,RotationEdt] = sliderPanel(...
    'Parent'  , PreOptsPanel, ...
    'Title'   , 'Rotation', ...
    'Position', [Hobjpos(1) 0.45 Hobjdim 0.4], ...
    'Min'     , -2, ...
    ...
    'Fontsize',8,...
    'Callback', @ImgRotate);
set(RotationSlider,'TooltipString',tts);
set(RotationPanel,'Bordertype','none');

tts = 'Scale image.';
tts = sprintf(wrapString(tts,ttStringLength));
[ScaleSlider, ScalePanel,ScaleEdt] = sliderPanel(...
    'Parent'  , PreOptsPanel, ...
    'Title'   , 'Scale', ...
    'Position', [Hobjpos(2) 0.45 Hobjdim 0.4], ...
    'Min'     , 0, ...
    ...
    'Fontsize',8,...
    'Callback', @ImgScale);
set(ScaleSlider,'TooltipString',tts);
set(ScalePanel,'Bordertype','none');

ico = im2double(imread(fullfile(matlabroot,'toolbox','shared','dastudio','resources','glue','Toolbars','16px','Up_16.png')));
icoud = cat(3,flipud(ico(:,:,1)),flipud(ico(:,:,2)),flipud(ico(:,:,3)));
icort = cat(3,icoud(:,:,1)',icoud(:,:,2)',icoud(:,:,3)');
icolt = cat(3,ico(:,:,1)',ico(:,:,2)',ico(:,:,3)');%
TranslateButtonUp = uicontrol(...
    'Parent',PreOptsPanel,...
    'Position',[Hobjpos(3)+0.1 0.45 0.05 0.15],...
    'Fontname','Symbol',...
    'Fontweight','bold',...
    'String','',...%'Ù'
    'Cdata',ico,...
    'Style','pushbutton',...
    'TooltipString','Shift image up',...
    'Enable','on',...
    'Callback',@TranslateUp,...
    'Tag','falsecolor');

TranslateButtonDown = uicontrol(...
    'Parent',PreOptsPanel,...
    'Position',[Hobjpos(3)+0.1 0.25 0.05 0.15],...
    'Fontname','Symbol',...
    'Fontweight','bold',...
    'String','',...%'Ù'
    'Cdata',icoud,...
    'Style','pushbutton',...
    'TooltipString','Shift image down',...
    'Enable','on',...
    'Callback',@TranslateDown,...
    'Tag','falsecolor');

TranslateButtonRight = uicontrol(...
    'Parent',PreOptsPanel,...
    'Position',[Hobjpos(3)+0.16 0.325 0.05 0.15],...
    'Fontname','Symbol',...
    'Fontweight','bold',...
    'String','',...%'Ù'
    'Cdata',icort,...
...
TranslateButtonLeft = uicontrol(...
    'Parent',PreOptsPanel,...
    'Position',[Hobjpos(3)+0.04 0.325 0.05 0.15],...
    'Fontname','Symbol',...
    'Fontweight','bold',...
    'String','',...%'Ù'
    'Cdata',icolt,...
    ...
    'TooltipString','Shift image down',...
    'Enable','on',...
    'Callback',@TranslateLeft,...
    'Tag','falsecolor');

uicontrol(...
    'Parent',PreOptsPanel,...
    'Callback','',...
    'Position',[Hobjpos(3)+0.05 0.7 0.475 0.1],...
    'String','Translation',...
    'Fontweight','bold',...
    'Foregroundcolor',fgc,...
    'Style','text',...
...
FlipButton = uicontrol(...
    'Parent',PreOptsPanel,...
    'Callback',@ImgFlip,...
    'FontSize',12,...
    'Fontweight','b',...
    'Position',[0.05 0.175 0.3 0.15],...
    'String','Flip Image',...
    'Tooltipstring','Flip image',...
    'Tag','FlipButton');

FlipButton = uicontrol(...
    'Parent',PreOptsPanel,...
    'Callback',@SavePreprocessedImage,...
    'FontSize',12,...
    'Fontweight','b',...
    'Position',[0.35 0.175 0.3 0.15],...
    'String','Save Image',...
    'Tooltipstring','Save preprocessed image for analysis',...
    'Tag','SaveButton');
...
% -----------------SUBFUNCTIONS-------------------------------------
	...
	function changeTformType(varargin)
		...
		if strcmp(cbstring,'Preprocess')
            set(PreOptsPanel,'visible','on')
            set(MultimodalOptsPanel,'visible','off')
            set(MonomodalOptsPanel,'visible','off')
        
        elseif strcmp(cbstring,'Monomodal')
			set(MultimodalOptsPanel,'visible','off')
            set(PreOptsPanel,'visible','off')
    ...
    function SavePreprocessedImage(varargin)
            visualizeResults;
            save('tform_initial','T');
            movingInputImage = registered;
            img_size = size(movingInputImage);
            sizeX = img_size(2)
            sizeY = img_size(1);
%             fixedInputImage = imcrop(fixedInputImage,[1 1 sizeY+VerticalTranslation sizeX+HorizontalTranslation]);
            
        end
        
    function ImgRotate(varargin)
        theta = get(RotationSlider,'value');
%         registered = imrotate(movingInputImage,theta,'bilinear','crop');
        visualizeResults;
    end

    function ImgScale(varargin)
        scale = get(ScaleSlider,'value');
%         registered = imresize(movingInputImage,scale);
        visualizeResults;
    end

    function TranslateUp(varargin)
        VerticalTranslation = VerticalTranslation-StepSize;
        visualizeResults;
    end
    function TranslateDown(varargin)
        VerticalTranslation = VerticalTranslation+StepSize;
        visualizeResults;
    end
    function TranslateRight(varargin)
        HorizontalTranslation = HorizontalTranslation+StepSize;
        visualizeResults;
    end
    function TranslateLeft(varargin)
        HorizontalTranslation = HorizontalTranslation-StepSize;
        visualizeResults;
    end
    function ImgFlip(varargin)
         flip = -flip;
         visualizeResults;
    ...
    function visualizeResults(varargin)
        ...
        OptsType = getButtonGroupVal(RegistrationConfigButtonGroup,'tag');
        ...
        if strcmp(OptsType,'Preprocess')
            %do rotation, scaling, translation
%             
%             if ~doflip
...
%             end
            ...
            T = affine2d(tform);
            T2 = maketform('affine',tform);
            RotatedImage = imrotate(movingInputImage,theta,'bilinear','crop');
            ScaledImage = imresize(RotatedImage,scale);
%             
%             registered = imtranslate(ScaledImage,[HorizontalTranslation,VerticalTranslation]);
            sizeFixed = size(fixedInputImage);
            registered = imtransform(movingInputImage,T2,'XData',...
                [1 size_fixedInputImage(2)], 'YData', [1 size_fixedInputImage(1)],...
                'FillValues',0, 'Size', sizeFixed);
