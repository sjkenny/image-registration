...
if ~single_list
    ...
    hm_x_use = r_hm.xc(idx_1b);
    hm_y_use = r_hm.yc(idx_1b);
else
...
%% cat2 for 1b, cat1 for 1s


[centers_heatmap,counts_heatmap,cluster_idx_heatmap] = dbscan_fcn(hm_x_use,hm_y_use,6,1.5);
...
while KeepGoing==1
    ...
    [count1 edges1 mid1 loc1] = histcn(X, 0:ceil(max(y_brp_filter)), 0:ceil(max(x_brp_filter)));
...
for j=1:length(hm_match_final)
    ...
    dist(j) = l2_dist_mat(centers_heatmap(heatmap_idx_now,:)',centers_brp(storm_idx_now,:)');
...
% end
%% filter outliers
% match_idx_filter is used to reference back to originial centers list
...
filenameStats = sprintf('%s-stats.mat',filehead_hm);
